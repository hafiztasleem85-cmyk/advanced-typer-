    ...
    Http.get(versionUrl .. "?t=" .. timestamp, nil, "utf-8", nil, function(code, res)
        if code == 200 and res then
            ...
            if isUpdateAvailable(currentVersion, onlineVersion) then
                ...
                local function showUpdateError(errorText)
                ...
                local function showUpdateSuccess(newVer)
                    Handler(Looper.getMainLooper()).post(Runnable({
                        run = function()
                            local alert = AlertDialog.Builder(service)
                ...
                Http.get(notesUrl .. "?t=" .. timestamp, nil, "utf-8", nil, function(codeNotes, resNotes)
                    local updateMessage = "New update available"
...
local function showSettings()
    ...
    globalMainDialog = builder.create()
    globalMainDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY)
    ...
    textOfflineDesc.setText("Offline voice typing allows you to type without an internet connection. The system will automatically switch to offline mode when there is no internet, and it will switch back to online mode when the internet is available. The language you select for voice dictation in the main menu will automatically be used for offline typing.\n\nTo use this feature, the following conditions are mandatory:\n1. You must download the voice pack for your selected language by clicking the Download Package button below.\n2. If you do not find the option to download the voice pack upon clicking this button, you can access it directly through your phone's settings. Go to your phone's Settings, navigate to Additional Settings, System Settings, or Keyboard & Input Method, and look for Google Voice Typing. From there, open Manage Offline Languages to download your pack.\n3. Please ensure that the downloaded voice pack exactly matches your selected voice dictation language. For example, if you have selected English India, you must download the English India pack. Having the English US pack will prevent offline typing from working.")
    ...
    globalMainDialog.show()
    globalMainDialog.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_ALT_FOCUSABLE_IM)
    globalMainDialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE)
    titleMic.sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_FOCUSED)
end
...
local function startListening(node, useShortTag)
    ...
    if not isConnected then
        if useShortTag then
            activeLangCode = baseLanguages[conf.micIndex + 1][3]
            if not activeLangCode or activeLangCode == "" then
                activeLangCode = baseLanguages[conf.micIndex + 1][2]
            end
        else
            activeLangCode = baseLanguages[conf.micIndex + 1][2]
        end
    ...
    local speechListener = RecognitionListener{
        onResults = function(results)
            ...
            if data and data.size() > 0 then
                ...
                local triggers = {"switch language ", "स्विच लैंग्वेज ", "سوئچ لینگویج ", "سوچ لینگویج ", "تبديل اللغة ", "سويتش سويتش ", "سويتش ", "سويس لانج ", "سويتش لانج ", "سويت ", "سويت لانجز ", "سويتش لانجويج "}
                ...
                if not conf.translate then
                    local detailCommands = {
                        ...
                        ["माय मोबाइल नंबर"] = conf.details["mobile"],
        ...
        onError = function(errorCode)
            if not isConnected then
                if not useShortTag and errorCode ~= SpeechRecognizer.ERROR_NO_MATCH and errorCode ~= SpeechRecognizer.ERROR_SPEECH_TIMEOUT then
                    safeStop(speechRecognizer)
                    Handler(Looper.getMainLooper()).postDelayed(Runnable{
                        run = function()
                            pcall(function() startListening(node, true) end)
                        end
                    }, 200)
                    return
                end
